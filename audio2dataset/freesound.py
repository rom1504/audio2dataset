...
sets = askdirectory(title='Select a directory with parquet sets')
save_folder = askdirectory(title='Select a directory to save the downloaded files')
for file in os.listdir(sets):
    df = pd.read_parquet(sets + '/' + file)
