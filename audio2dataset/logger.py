...
class SpeedLogger(Logger):
    ...
    def __call__(
        self, count, success, failed_to_download, failed_to_subsample, start_time, end_time
        ...
        super().__call__(
            self.count, self.success, self.failed_to_download, self.failed_to_subsample, self.start_time, self.end_time
        )
    ...
    def do_log(
        ...
        print(
            " - ".join(
                [
                    ...
                    f"failed to subsample: {failed_to_subsample_ratio:.3f}",
                    f"audios per sec: {audio_per_sec:.0f}",
                    f"count: {count}",
                ]
            )
        )
        ...
        if self.enable_wandb:
            wandb.log(
                {
                    f"{self.prefix}/audio_per_sec": audio_per_sec,
                    f"{self.prefix}/success": success_ratio,
                    f"{self.prefix}/failed_to_download": failed_to_download_ratio,
                    f"{self.prefix}/failed_to_subsample": failed_to_subsample_ratio,
...
def write_stats(
    ...
    stats = {
        ...
        "failed_to_subsample": failed_to_subsample,
        "duration": end_time - start_time,
                    ...
                    with fs.open(stats_file, "r") as f:
                        try:
                            ...
                            SpeedLogger("worker", enable_wandb=self.enable_wandb)(
                                ...
                                failed_to_subsample=stats["failed_to_subsample"],
