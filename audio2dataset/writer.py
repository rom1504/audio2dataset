""""writer module handle writing the audios to disk"""
...
class ParquetSampleWriter:
    """ParquetSampleWriter is a audio+caption writer to parquet"""

    def __init__(self, shard_id, output_folder, save_caption, oom_shard_count, schema):
        self.oom_shard_count = oom_shard_count
        schema = schema.append(pa.field("flac", pa.binary()))
...
class WebDatasetSampleWriter:
    """WebDatasetSampleWriter is a audio+caption writer to webdataset"""
    ...
    def write(self, audio_str, key, caption, meta):
        ...
        if audio_str is not None:
            sample = {"__key__": key, "flac": audio_str}
            if self.save_caption:
                sample["txt"] = str(caption) if caption is not None else ""
...
class TFRecordSampleWriter:
    """TFRecordSampleWriter is a audio+caption writer to TFRecord"""
    ...
    def write(self, audio_str, key, caption, meta):
        ...
        if audio_str is not None:
            sample = {
                ...
                "flac": self._bytes_feature(audio_str),
            }
            if self.save_caption:
...
class FilesSampleWriter:
    """FilesSampleWriter is a caption+audio writer to files"""
    ...
    def write(self, audio_str, key, caption, meta):
        ...
        if audio_str is not None:
            filename = f"{self.subfolder}/{key}.flac"
