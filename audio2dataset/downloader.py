...
def download_audio(row, timeout):
    """Download an audio with urllib"""
    key, url = row
    audio_stream = None
    try:
        ...
        with urllib.request.urlopen(request, timeout=timeout) as r:
            audio_stream = io.BytesIO(r.read())
        return key, audio_stream, None
...
def download_audio_with_retry(row, timeout, retries):
    for _ in range(retries + 1):
        key, audio_stream, err = download_audio(row, timeout)
        if audio_stream is not None:
            return key, audio_stream, err
    return key, None, err
...
class Downloader:
    ...
    def download_shard(
        ...
        with ThreadPool(self.thread_count) as thread_pool:
            for key, audio_stream, error_message in thread_pool.imap_unordered(
                lambda x: download_audio_with_retry(x, timeout=self.timeout, retries=self.retries),
                loader,
            ):
                try:
                    ...
                    if self.compute_md5:
                        audio_stream.seek(0)
                        meta["md5"] = hashlib.md5(audio_stream.read()).hexdigest()
